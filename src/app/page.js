...
export default function OfertaGenPage() {
  ...
  const doGenerateWord = useCallback(async () => {
    ...
    setGenerating(false);
  }, [bloques, data.partes.ofertante.personas, logoBase64, lang2, data.bloques.firmas_en_linea]);
      ...
      <div className="bg-white rounded-2xl p-6 mb-4 min-h-[400px]" style={{background:"var(--og-card)",border:"1px solid var(--og-border)"}}>
        ...
        {step === 2 && <>
          <Section title={t.sections.precio}>
            <Toggle
              label="Precio compuesto (Inmueble + Muebles) / Composite Price (Property + Furnishings)"
              sub="Divide el precio en dos operaciones: escritura pública (inmueble) + contrato privado (muebles)"
              checked={data.bloques.precio_compuesto}
              onChange={() => togBloque('precio_compuesto')}
            />
            {data.bloques.precio_compuesto ? (
              <>
                <Input label={`Precio del inmueble (${data.campos.precio?.moneda||"USD"}) / Real property price`} value={data.campos.precio?.precio_inmueble||""} onChange={v=>upCampo("precio","precio_inmueble",v)} type="number" required hasError={fieldErrors["campos.precio.precio_inmueble"]} />
                <Input label={`Precio de muebles (${data.campos.precio?.moneda||"USD"}) / Personal property price`} value={data.campos.precio?.precio_muebles||""} onChange={v=>upCampo("precio","precio_muebles",v)} type="number" required hasError={fieldErrors["campos.precio.precio_muebles"]} />
                ...
                <div className="col-span-2">
                  <Toggle
                    label="Pactar que el valor de los muebles no se incluya en la Escritura"
                    sub="Obligacion bilateral de omision documental sobre el instrumento publico. Revisar antes de activar: obliga tambien a la parte vendedora."
                    checked={data.bloques.pacto_no_incluir_muebles}
                    onChange={() => togBloque('pacto_no_incluir_muebles')}
                  />
                </div>
              </>
            ) : (
            ...
            <div className="flex flex-col gap-1">
              ...
              <select value={data.campos.precio?.anticipo_gastos||"0"} onChange={e=>upCampo("precio","anticipo_gastos",e.target.value)} className="border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white">
                ...
                <option value="50pct">50% del estimado de gastos</option>
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs font-medium" style={{color:"var(--og-secondary)"}}>Tipo de cambio / Exchange rate</label>
              <select value={data.campos.precio?.tipo_cambio||"financiero"} onChange={e=>upCampo("precio","tipo_cambio",e.target.value)} className="border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white" style={{background:"var(--og-surface)",borderColor:"var(--og-border)",color:"var(--og-text)"}}>
                <option value="financiero">El Financiero</option>
        ...
        {step === 3 && <div className="space-y-3">
          ...
          <CatGroup title="Estructura de la operación" defaultOpen={true}>
            ...
            <Toggle label="Cuenta Escrow" sub={`Depósito condicional irrevocable (${data.campos.escrow?.empresa_escrow === "otro_escrow" ? (data.campos.escrow?.empresa_escrow_manual || "empresa escrow") : (data.campos.escrow?.empresa_escrow || "ARMOUR SECURE ESCROW, S DE RL DE CV")})`} checked={data.bloques.escrow} onChange={()=>togBloque("escrow")} />
          ...
          <CatGroup title="Legales y formalidades">
            ...
            <Toggle label="Gravamen vigente por cancelar" sub="Hipoteca inscrita todavía viva: carta de saldo del acreedor y cancelación previa o simultánea a la firma" checked={data.bloques.gravamen_por_cancelar} onChange={()=>togBloque("gravamen_por_cancelar")} />
            {data.bloques.gravamen_por_cancelar ? (
              <>
                <Input label="Acreedor del gravamen" value={data.campos.gravamen?.acreedor||""} onChange={v=>upCampo("gravamen","acreedor",v)} placeholder="Hipotecaria Nacional, S.A. de C.V. / SHF" />
                <Input label="Días hábiles de anticipación para la carta de saldo" value={data.campos.gravamen?.dias_carta_saldo||10} onChange={v=>upCampo("gravamen","dias_carta_saldo",v)} type="number" />
              </>
            ) : null}
            ...
            <Toggle label="Firmas en línea (formato DOCX)" sub="Coloca las dos firmas lado a lado; compacta la última página. Solo afecta el Word." checked={data.bloques.firmas_en_linea} onChange={()=>togBloque("firmas_en_linea")} />
          </CatGroup>
